...
@property (nonatomic, assign) NSInteger topImageIndex;
...
+ (instancetype)cycleScrollViewWithFrame:(CGRect)frame delegate:(id<SDCycleScrollViewDelegate>)delegate placeholderImage:(UIImage *)placeholderImage Type:(NSInteger )type
{
    SDCycleScrollView *cycleScrollView = [[self alloc] initWithFrame:frame];
    cycleScrollView.delegate = delegate;
    cycleScrollView.placeholderImage = placeholderImage;
    cycleScrollView.lytype = type;
    return cycleScrollView;
}
...
-(void)setLytype:(NSInteger)lytype{
    _lytype = lytype;
}
...
- (void) addTopImageWithImageName:(NSString *)imgName atIndex:(NSInteger)index {
    self.topImageName = imgName;
    self.topImageIndex = index;
    if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
        TAPageControl *pageControl = (TAPageControl *)_pageControl;
        pageControl.shouldShowVideoSign = YES;
        pageControl.videoIndex = index;
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            if (self.topImageName && itemIndex == self.topImageIndex) {
                [cell.imageView setImageWithUrl:imagePath placeholder:self.placeholderImage];
    ...
    if (self.topImageName && itemIndex == self.topImageIndex) {
        cell.topImageName = self.topImageName;
    }else {
        cell.topImageName = nil;
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:didSelectItemAtIndex:)]) {
        if (_lytype>0) {
            [self.delegate cycleScrollView:self didSelectItemAtIndex:indexPath.item % self.imagePathsGroup.count Type:_lytype];
        }else{
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if ([self.delegate respondsToSelector:@selector(cycleScrollView:willScrollFromIndex:)]) {
        [self.delegate cycleScrollView:self willScrollFromIndex:indexOnPageControl];
    }
}
